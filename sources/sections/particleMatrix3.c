...
} particleMatrix3_section;
	
static particleMatrix3_section *local;
...
void load_particleMatrix3() {
	...
	local = (particleMatrix3_section*) malloc(sizeof(particleMatrix3_section));
	memset(local, 0, sizeof(particleMatrix3_section));
	...
	if (!local->object) {
		section_error("particleMatrix3: Error loading %s", mySection->strings[0]);
		return;
	}
	...
	rateEquation = (char*) calloc(equationSize+1, sizeof(char));
	strcpy(rateEquation, (char *) mySection->strings[1]); //-V525
	strcat(rateEquation, (char *) mySection->strings[10]);
	strcat(rateEquation, (char *) mySection->strings[11]);
	local->myParticleSystem->rate.equation = rateEquation;
	...
	evalEquation = (char*)calloc(equationSize+1, sizeof(char));
	...
	evalSources = (char*)calloc(equationSize+1, sizeof(char));
...
void init_particleMatrix3 () {
	local = (particleMatrix3_section *) mySection->vars;
